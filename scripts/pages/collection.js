(function () {
    ...
    const activateTab = (tabName, { moveFocus = false, updateHash = false } = {}) => {
        const tab = tabs.find((item) => item.dataset.collectionTab === tabName);
        ...
        panels.forEach((panel) => {
            const isActive = panel.dataset.collectionPanel === tabName;
    ...
    tabs.forEach((tab, index) => {
        tab.addEventListener('click', () => {
            activateTab(tab.dataset.collectionTab, { updateHash: true });
        });
        ...
        tab.addEventListener('keydown', (event) => {
            ...
            activateTab(tabs[nextIndex].dataset.collectionTab, {
                moveFocus: true,
                updateHash: true,
            });
        });
    ...
    const defaultTabName = initialTab.dataset.collectionTab;
